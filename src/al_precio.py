...
import time
import gradio as gr
from framework_agentes import DealAgentFramework
...
class App:
    ...
    def run(self):
        with gr.Blocks(title="Al Precio", fill_width=True) as ui:
            ...
            def get_initial_plot():
                fig = go.Figure()
                fig.update_layout(
                    title='Cargando la base de vectores...',
                    height=400,
                )
                return fig
            ...
            with gr.Row():
                gr.Markdown('<div style="text-align: center;font-size:24px"><strong>Al Precio</strong> - Framework autónomo de agentes en busca de ofertas en línea</div>')
            # with gr.Row():
                # gr.Markdown('<div style="text-align: center;font-size:14px">Un LLM propietario afinado desplegado en Modal, junto con una canalización RAG y un modelo frontier, colaboran para enviar notificaciones push con excelentes ofertas en línea.</div>')
            with gr.Row():
                opportunities_dataframe = gr.Dataframe(
                    headers=["Ofertas encontradas", "Precio", "Estimación", "Descuento", "URL"],
                    wrap=True,
